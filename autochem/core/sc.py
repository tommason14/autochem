...
class Supercomp:
    ...
    def __init__(self):
        ...
        hostname = subprocess.run("hostname", encoding="utf-8", stdout=subprocess.PIPE).stdout[:-1]
        for key in cases:
            if key in hostname:
                self.sc = cases[key]
                break
    ...
    def __add__(self, other):
        ...
        if isinstance(other, str):
            ...
            raise TypeError(
                "Can only concatenate Supercomp instances with strings. No addition to floats/integers"
            )
