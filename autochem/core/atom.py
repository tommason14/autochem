...
class Atom:
    ...
    * ``atomic_number`` -- atomic number, equal to zero for a dummy atom
    ...
    * ``symbol`` -- read or write the atom symbol directly
    ...
    def __init__(self, symbol=None, atomic_number=0, coords=None, mol=None, bonds=None):
        if symbol is not None:
            self.symbol = symbol
            self.atomic_number = PT.get_atomic_number(self.symbol)
        else:
            self.atomic_number = atomic_number
            self.symbol = PT.get_symbol(self.atomic_number)

        self.mass = PT.get_mass(self.symbol)
        self.mol = mol
        self.bonds = bonds or []
