...
def calculate_interaction_energies(csv, with_ionic=False, pretty_print=False, output=None):
    """
    ...
    if with_ionic:
        ...
        df = df.melt(id_vars=retain, value_vars=["HF", "Corr"], value_name="values", var_name="energy")
        df["energy_type"] = df["energy"] + "-" + df["Type"]
        # Create groupby variable (path of each cluster) after pivoting
        df = df.pivot(index="Path", columns="energy_type", values="values").reset_index()
        ...
        df = df.reset_index()
        df["HF_int_KJ_mol"] = (df["HF-complex"] - df["HF-ionic"] - df["HF-frag"]) * HART_TO_KJ
        df["Corr_int_KJ_mol"] = (df["Corr-complex"] - df["Corr-ionic"] - df["Corr-frag"]) * HART_TO_KJ
        ...
        # sum to remove NA values
        df = df.groupby("Config").agg(
            {"Corr-complex": "sum", "Corr-frag": "sum", "HF-complex": "sum", "HF-frag": "sum"}
        )
        # groupby just makes the Config column and index by which rows are collected,
        # reset the index to get the column back
        df = df.reset_index()
        ...
        df["Total_int_KJ_mol"] = df["HF_int_KJ_mol"] + df["Corr_int_KJ_mol"]
    if pretty_print:
        df_ = df[["Config", "HF_int_KJ_mol", "Corr_int_KJ_mol", "Total_int_KJ_mol"]].to_dict(orient="list")
...
def apply_boltzmann_weightings(csv, grouping, pretty_print=False, output=None):
    ...
    weighted_energies = df.groupby("Groups").agg({"HF-weighted": "sum", "Corr-weighted": "sum"})
    weighted_energies = weighted_energies.rename(
        columns={"HF-weighted": "HF", "Corr-weighted": "Corr"}
    ).reset_index()

    hf_confidence_intervals = (
        df.groupby("Groups")["HF-weighted"].apply(_confidence).reset_index(name="HF_confidence_interval")
    )
    corr_confidence_intervals = (
        df.groupby("Groups")["Corr-weighted"].apply(_confidence).reset_index(name="Corr_confidence_interval")
    )
